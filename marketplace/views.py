...
def user_login(request):
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            # **check if it is superuser or staff**
...
def edit_item(request, item_id):
    ...
    # only superuser add seller can edit it
...
def delete_item(request, item_id):
    ...
    # only superuser add seller can delete it
    if not (request.user == item.seller or request.user.is_staff or request.user.is_superuser):
...
def message_center(request, receiver_id=None):
    ...
    # **Get the most recent messages (in descending chronological order, most recent first)**
    ...
    if receiver_id is None and unique_users:
        return redirect('message_center_with_id', receiver_id=unique_users[0].id)  
